...
export default function HomePage() {
  ...
  const spacingClass = layout === 'compact' ? 'py-12' : 'py-20';
    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className={`relative z-10 text-center px-4 sm:px-6 lg:px-8 ${containerClass} mx-auto`}>
          <h1 className="text-5xl md:text-7xl font-light tracking-tight text-white mb-6 animate-fade-in">
            {schoolName}
            <span className="block text-accent-400">{schoolTagline}</span>
          </h1>
          <p className="text-xl md:text-2xl text-gray-200 mb-12 font-light leading-relaxed max-w-3xl mx-auto animate-fade-in-delay">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fade-in-delay-2">
            <button className="group bg-accent-600 text-white px-8 py-4 rounded-full text-lg font-medium hover:bg-accent-700 transition-all duration-300 flex items-center justify-center space-x-2">
              <span>Explore Programs</span>
              <ArrowRight className="h-5 w-5 group-hover:translate-x-1 transition-transform" />
            </button>
            <button className="border-2 border-white/30 text-white px-8 py-4 rounded-full text-lg font-medium hover:bg-white/10 transition-all duration-300 backdrop-blur-sm">
              Take a Virtual Tour
            </button>
          </div>
        </div>
